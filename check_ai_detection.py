...
def check_ai_detection():
    ...
    try:
        ...
        response = requests.post(f'http://localhost:8000/analyze/{job_id}', json=analyze_data, timeout=30)
        if response.status_code != 200:
        ...
        for i in range(30):  # 最多等待60秒
            ...
            if response.status_code == 200:
                ...
                if status['status'] == 'done':
                    ...
                    # 4. 获取结果
                    response = requests.get(f'http://localhost:8000/jobs/{job_id}/result', timeout=10)
                    if response.status_code == 200:
                        result = response.json()
