...
class RuleFindingsService:
    ...
    async def analyze(self, context: JobContext) -> List[IssueItem]:
        ...
        try:
            ...
            logger.info(
                f"规则分析完成: job_id={context.job_id}, issues={len(issues)}, elapsed={elapsed:.2f}s"
            )
    ...
    def _convert_to_issues(
        self, rule_results: List[Dict[str, Any]], context: JobContext
    ) -> List[IssueItem]:
    ...
    def _convert_single_result(
        self, result: Dict[str, Any], context: JobContext, idx: int
    ) -> Optional[IssueItem]:
        """转换单个规则结果"""
        try:
            # 提取基本信息
