...
class AIClientV2:
    ...
    async def chat_completion(
        ...
        **kwargs,
        ...
        for model_config in failover_models:
            try:
                logger.info(
                    f"尝试使用模型: {model_config.provider.value}:{model_config.model_name}"
                )
                ...
                if model_config.provider == ModelProvider.ZHIPU:
                    response = await self._call_zhipu(
                        model_config, messages, temperature, max_tokens, timeout, **kwargs
                    )
                ...
                logger.info(
                    f"模型调用成功: {model_config.provider.value}:{model_config.model_name}, 耗时: {latency_ms}ms"
                )
                ...
                last_error = e
                logger.warning(
                    f"模型 {model_config.provider.value}:{model_config.model_name} 调用失败: {str(e)}"
                )
                continue
        ...
        **kwargs,
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            response = await client.post(
                f"{model_config.base_url}/chat/completions", headers=headers, json=payload
            )
    ...
    async def stream_chat(
        ...
        **kwargs,
        ...
        try:
            if model_config.provider == ModelProvider.ZHIPU:
                async for chunk in self._stream_zhipu(
                    model_config, messages, temperature, max_tokens, timeout, **kwargs
                ):
                    yield chunk
            elif model_config.provider == ModelProvider.DEEPSEEK:
        ...
        **kwargs,
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            async with client.stream(
                "POST", f"{model_config.base_url}/chat/completions", headers=headers, json=payload
            ) as response:
                if response.status_code != 200:
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            async with client.stream(
                ...
                if response.status_code != 200:
                    error_text = await response.aread()
                    raise AIError(
                        f"DeepSeek流式调用失败 (HTTP {response.status_code}): {error_text}"
                    )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        for model_config in models[:2]:  # 只测试前2个模型
            ...
            try:
                ...
                result["tested_models"][model_key] = {"status": "unhealthy", "error": str(e)}
