...
class FindingsMerger:
    ...
    def merge_findings(
        self, ai_findings: List[IssueItem], rule_findings: List[IssueItem]
    ) -> MergedSummary:
        """合并两路结果"""
        ...
        agreements = [
            ...
            if not self._is_conflict_match(matches[i], conflicts)
        ]
        ...
        return MergedSummary(
            totals=totals, conflicts=conflicts, agreements=agreements, merged_ids=merged_ids
        )
    ...
    def _check_item_conflict(
        self, ai_item: IssueItem, rule_item: IssueItem, key: str
    ) -> ConflictItem:
        """检查单个匹配对是否冲突"""
        conflicts = []
        details = {}
        ...
        if ai_item.severity != rule_item.severity:
            ...
            if abs(ai_level - rule_level) >= 2:  # 相差2级以上认为是冲突
                ...
                details["severity"] = {"ai": ai_item.severity, "rule": rule_item.severity}
        ...
        if ai_page > 0 and rule_page > 0 and abs(ai_page - rule_page) > self.config.page_tolerance:
            ...
            details["page"] = {"ai": ai_page, "rule": rule_page}
        ...
        for key in common_keys:
            ...
            if isinstance(val1, (int, float)) and isinstance(val2, (int, float)):
                if val1 != 0 and val2 != 0:
                    rel_diff = abs(val1 - val2) / max(abs(val1), abs(val2))
                    if rel_diff > self.config.money_tolerance:
                        conflicts[key] = {"ai": val1, "rule": val2, "diff_pct": rel_diff * 100}
...
def merge_findings(
    ai_findings: List[IssueItem], rule_findings: List[IssueItem], config: AnalysisConfig
) -> MergedSummary:
    """合并结果的便捷函数"""
