...
class EvidenceEnhancer:
    ...
    def merge_nearby_hits(
        self, coordinates: List[EvidenceCoordinate], merge_distance: float = 100.0
    ) -> List[List[EvidenceCoordinate]]:
        """
        合并相近的命中项
    ...
    def expand_to_complete_sentences(
        self, extractor: "EvidenceExtractor", coordinates: List[EvidenceCoordinate]
    ...
    def create_paginated_evidence(
        self, merged_groups: List[List[EvidenceCoordinate]], items_per_page: int = 10
    ) -> List[Dict[str, Any]]:
        """
        创建分页的证据展示
    ...
    def _calculate_group_bounding_box(self, group: List[EvidenceCoordinate]) -> Dict[str, float]:
        ...
        return {"x1": min_x1, "y1": min_y1, "x2": max_x2, "y2": max_y2}
...
class EvidenceExtractor:
    ...
    def capture_screenshot(
        ...
        try:
            ...
            # 在原图上添加红框高亮（可选）
            if highlight_color:
                self._add_highlight_to_image(
                    image_path, evidence_rect, screenshot_rect, highlight_color, mat.a
                )
    ...
    def _add_highlight_to_image(
        self,
        image_path: Path,
        evidence_rect: fitz.Rect,
        screenshot_rect: fitz.Rect,
        ...
        try:
            ...
            with Image.open(image_path) as img:
                ...
                draw.rectangle([rel_x1, rel_y1, rel_x2, rel_y2], outline=rgb_color, width=3)
    ...
    def extract_evidence_batch(
        self, text_list: List[str], enable_screenshots: bool = True
    ) -> Dict[str, Any]:
        """
        批量提取证据坐标和截图
        ...
        logger.info(
            f"Batch extraction completed: {len(results['coordinates'])} coordinates, "
            f"{len(results['screenshots'])} screenshots"
        )

        return results

    def extract_evidence_enhanced(
        self,
        text_list: List[str],
        ...
        if enable_sentence_expansion:
            ...
            all_coordinates = self.enhancer.expand_to_complete_sentences(self, all_coordinates)

        # 命中合并
        merged_groups = []
        if enable_hit_merging:
            logger.info("Merging nearby hits...")
            merged_groups = self.enhancer.merge_nearby_hits(all_coordinates, merge_distance)
        else:
            # 每个坐标单独成组
            merged_groups = [[coord] for coord in all_coordinates]

        # 分页展示
        paginated_evidence = self.enhancer.create_paginated_evidence(merged_groups, items_per_page)
        ...
        logger.info(
            f"Enhanced extraction completed: {len(all_coordinates)} coordinates, "
    ...
    def create_evidence_zip(self, job_id: str, evidence_data: Dict[str, Any]) -> str:
        ...
        try:
            with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
                ...
                zipf.writestr(
                    "coordinates.json",
                    json.dumps(evidence_data.get("coordinates", []), ensure_ascii=False, indent=2),
                )
