...
class PerformanceOptimizer:
    ...
    def start_task(self, task_id: str, file_info: Dict[str, Any]) -> TaskMetrics:
        ...
        metrics = TaskMetrics(task_id=task_id, start_time=time.time(), status="running")
        ...
        logger.info(
            f"任务 {task_id} 已启动，预估完成时间: {metrics.estimated_remaining_seconds:.1f}秒"
        )
        return metrics
        ...
        try:
            ...
            return {
                ...
                "recent_completions": len(
                    [t for t in self.task_history if time.time() - t.start_time < 3600]
                ),  # 最近1小时完成的任务
            }
        except Exception as e:
    ...
    def check_resource_limits(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if memory_used_mb > self.config.memory_limit_mb * 0.9:
                warnings.append(
                    f"内存使用接近限制: {memory_used_mb:.1f}MB / {self.config.memory_limit_mb}MB"
                )
    ...
    def cleanup_expired_tasks(self):
        ...
        for task_id, metrics in self.active_tasks.items():
            ...
            timeout = (
                ...
                if metrics.memory_usage_mb > self.config.large_file_threshold_mb
...
def optimize_analysis_pipeline(file_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "estimated_duration": optimizer.estimate_task_duration(
            file_size_mb, file_info.get("pages", 0)
        ),
    }
