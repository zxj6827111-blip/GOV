...
class DocumentSegmentProcessor:
    ...
    async def process_document_with_monitoring(
        self, document_path: str, job_id: str, processor_func: Callable, **kwargs
    ) -> Dict[str, Any]:
        """
        带监控的文档处理
        ...
        try:
            async with self.timeout_controller.timeout_context():
                ...
                logger.info(
                    f"开始处理文档: {document_path}, 总页数: {total_pages}, 文件大小: {file_size_mb:.1f}MB"
                )
    ...
    async def _execute_processor(self, processor_func: Callable, *args, **kwargs) -> Dict[str, Any]:
        ...
        if asyncio.iscoroutinefunction(processor_func):
            ...
            return await loop.run_in_executor(
                self.concurrency_controller.executor, processor_func, *args, **kwargs
            )
