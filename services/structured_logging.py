...
class BusinessLoggerAdapter(logging.LoggerAdapter):
    ...
    def log_stage_end(self, stage: ProcessingStage, duration_ms: float, **kwargs):
        """记录阶段结束"""
        self.info(f"完成 {stage.value} 阶段", extra={"duration_ms": duration_ms, **kwargs})

    def log_page_processed(self, page_no: int, total_pages: int, ocr_used: bool = False, **kwargs):
        """记录页面处理"""
        self.debug(
            f"处理页面 {page_no}/{total_pages}",
            extra={
    ...
    def log_ocr_summary(self, total_pages: int, ocr_triggered_pages: int, **kwargs):
        ...
        self.info(
            f"OCR处理总结: {ocr_triggered_pages}/{total_pages} ({ocr_rate:.1%})",
            extra={
                "total_pages": total_pages,
    ...
    def log_analysis_result(
        self, rules_triggered: int, ai_findings: int, total_findings: int, **kwargs
    ):
        """记录分析结果"""
        self.info(
            f"分析完成: 规则 {rules_triggered} 项, AI {ai_findings} 项, 总计 {total_findings} 项",
            extra={
    ...
    def log_performance(self, operation: str, duration_ms: float, memory_mb: float, **kwargs):
        """记录性能指标"""
        self.info(
            f"性能: {operation} 耗时 {duration_ms:.1f}ms, 内存 {memory_mb:.1f}MB",
            extra={"duration_ms": duration_ms, "memory_mb": memory_mb, **kwargs},
        )
...
def setup_structured_logging(
    log_file: Optional[str] = None, log_level: str = "INFO", enable_console: bool = True
...
class LogAnalyzer:
    ...
    def analyze_job_performance(self, job_id: str) -> Dict[str, Any]:
        ...
        for log in job_logs:
            ...
            if stage and log.get("duration_ms"):
                ...
                stages[stage]["avg_duration"] = (
                    stages[stage]["total_duration"] / stages[stage]["count"]
                )
        ...
        return {
            ...
            "stages": stages,
            "ocr_trigger_rate": ocr_stats["ocr_pages"] / ocr_stats["total_pages"]
            if ocr_stats["total_pages"] > 0
            else 0,
    ...
    def get_system_overview(self, hours: int = 24) -> Dict[str, Any]:
                    ...
                    if job_id:
                        ...
                        if log_entry.get("memory_mb"):
                            job_stats[job_id]["max_memory"] = max(
                                job_stats[job_id]["max_memory"], log_entry["memory_mb"]
                            )
        ...
        avg_memory = (
            sum(j["max_memory"] for j in job_stats.values()) / len(job_stats) if job_stats else 0
        )
        avg_duration = (
            sum(j["total_duration"] for j in job_stats.values()) / len(job_stats)
            if job_stats
            else 0
        )
...
def get_business_logger() -> BusinessLoggerAdapter:
    ...
    if business_logger is None:
        business_logger = setup_structured_logging(
            log_file="logs/govbudget.jsonl", log_level="INFO", enable_console=True
        )
