...
class RobustNumberParser:
    ...
    def _compile_patterns(self):
        """编译常用正则表达式"""
        # 阿拉伯数字（含千分位、小数点）
        self.arabic_pattern = re.compile(r"[+-]?\d{1,3}(?:,\d{3})*(?:\.\d+)?")

        # 括号负数（支持中文括号）
        self.bracket_negative_pattern = re.compile(r"[\(\uff08]([0-9,]+(?:\.[0-9]+)?)[\)\uff09]")

        # 百分号数字
        self.percent_pattern = re.compile(r"([0-9,]+(?:\.[0-9]+)?)%")

        # 中文数字 - 只匹配纯中文数字，不包含阿拉伯数字
        cn_digits = "".join(self.cn_num_map.keys())
        cn_units = "".join(self.cn_unit_map.keys())
        # 修复：移除阿拉伯数字0-9，避免误匹配
        self.chinese_pattern = re.compile(f"[{cn_digits}{cn_units}]+")

        # 跨行断词（数字被换行分割）
        self.cross_line_pattern = re.compile(r"(\d+)[，,]?\s*\n\s*(\d+)", re.MULTILINE)
    ...
    def calculate_tolerance(
        ...
        absolute_tolerance: Union[Decimal, float] = 0,
    ) -> bool:
        """
        计算两个数值是否在容差范围内 - 修复版本
    ...
    def extract_all_numbers(self, text: str) -> List[Tuple[str, Decimal, int, int]]:
        ...
        for pattern, _pattern_type in patterns:
            for match in pattern.finditer(text):
                original_text = match.group()
                parsed_value = self.parse_number(original_text)

                if parsed_value is not None:
                    results.append((original_text, parsed_value, match.start(), match.end()))
...
def calculate_tolerance(
    ...
    """便捷函数：容差计算"""
    return get_parser().calculate_tolerance(value1, value2, relative_tolerance, absolute_tolerance)
