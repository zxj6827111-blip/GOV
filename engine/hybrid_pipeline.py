...
class HybridPipeline:
    ...
    async def run_hybrid_validation(
        self, document: str, doc_obj: Any = None, use_ai_assist: bool = True
...
def convert_results_to_issues(results: List[ValidationIssue]) -> List[Issue]:
    ...
    for result in results:
        try:
            ...
            issue = Issue(
                rule=rule_code, severity=severity, message=result.description, location=location
            )
            issues.append(issue)
...
async def run_hybrid_rules(
    doc, document_text: str = "", use_ai_assist: bool = False
) -> List[Issue]:
    """
    异步运行混合验证规则
    ...
    try:
        ...
        results = await pipeline.run_hybrid_validation(
            document=document_text, doc_obj=doc, use_ai_assist=use_ai_assist
        )
...
def run_hybrid_rules_sync(doc, document_text: str = "", use_ai_assist: bool = False) -> List[Issue]:
    ...
    try:
        ...
        try:
            return loop.run_until_complete(run_hybrid_rules(doc, document_text, use_ai_assist))
        finally:
            loop.close()
