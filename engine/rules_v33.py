...
def tolerant_equal(
    a: Optional[float], b: Optional[float], atol: float = 1.0, rtol: float = 0.001
) -> bool:
...
NINE_ALIAS_NORMAL = [
    {"name": it["name"], "aliases_norm": [normalize_text(x) for x in it["aliases"]]}
        ...
        for spec in NINE_TABLES:
            ...
            if name_norm and (name_norm in ntxt or fuzz.partial_ratio(name_norm, ntxt) >= 92):
                ...
                for alias in spec.get("aliases", []):
                    ...
                    if alias_norm and (
                        alias_norm in ntxt or fuzz.partial_ratio(alias_norm, ntxt) >= 95
                    ):
                        matched = True
                        break
            if matched:
...
class R33001_CoverYearUnit(Rule):
    ...
    def apply(self, doc: Document) -> List[Issue]:
        ...
        if doc.dominant_year is None:
            issues.append(
                self._issue("未能在封面/目录或首个表页附近识别年度。", {"page": 1}, severity="warn")
            )
        all_units = [u for u in doc.units_per_page if u]
        if not all_units:
            issues.append(
                self._issue(
                    "未识别到金额单位（单位：万元/元/亿元）。", {"page": 1}, severity="warn"
                )
            )
        elif len(set(all_units)) > 1:
            issues.append(
                self._issue(
                    f"金额单位混用：{sorted(set(all_units))}。", {"page": 1}, severity="warn"
                )
            )
        return issues
    ...
    def apply(self, doc: Document) -> List[Issue]:
        ...
        if len(order_pages) >= 3:
            ...
            if indices != sorted(indices):
                msg = "九张表出现顺序可能异常（按首次出现页）。实际：" + " > ".join(
                    f"{nm}@{pg}" for nm, pg in actual
                )
...
class R33003_PageFileThreshold(Rule):
    ...
    def apply(self, doc: Document) -> List[Issue]:
        ...
        if doc.pages > 300:
            issues.append(
                self._issue(f"页数较多：{doc.pages} 页，建议分卷检查。", {"pages": doc.pages})
            )
        if doc.filesize > 50 * 1024 * 1024:
            mb = round(doc.filesize / (1024 * 1024), 1)
            issues.append(
                self._issue(
                    f"文件体积较大：{mb} MB，可能影响解析速度。", {"filesize": doc.filesize}
                )
            )
        return issues
...
def _row_value(
    table: List[List[str]], name_keys: Tuple[str, ...], prefer_cols: Tuple[str, ...] = ()
) -> Optional[float]:
    if not table:
        return None
    header = [str(x or "") for x in (table[0] if table else [])]
...
def _sum_by_func_class(table: List[List[str]], digits: int = 3) -> Dict[str, float]:
    ...
    if not table:
        return agg
    ncols = max(len(r) for r in table)
    for _r, row in enumerate(table[1:], start=1):
...
class R33102_TotalSheet_vs_Text(Rule):
    ...
    def apply(self, doc: Document) -> List[Issue]:
        try:
            ...
            return [
                self._issue(
                    "规则执行异常：maximum recursion depth exceeded", {"page": 1, "pos": 0}, "info"
                )
            ]
    ...
    def apply(self, doc: Document) -> List[Issue]:
        ...
        if p_txt is not None and total:
            ...
            if abs(p_calc - p_txt) > 1.0:
                issues.append(
                    self._issue(
                        f"财政拨款收入占比：表算{p_calc}% ≠ 文本{p_txt}%（容忍±1pct）",
                        {"page": p},
                        "warn",
                    )
                )
...
class R33104_Expense_vs_Text(Rule):
    ...
    def apply(self, doc: Document) -> List[Issue]:
        ...
        txt = "\n".join(doc.page_texts)
        for nm, a, b in [
            ("本年支出合计", total, near_number(txt, ["支出决算情况说明", "本年支出合计", "合计"])),
        ...
        for nm, a in [("基本支出", basic), ("项目支出", proj)]:
            ...
            if pct_t is not None and total:
                ...
                if abs(pct_c - pct_t) > 1.0:
                    issues.append(
                        self._issue(
                            f"{nm}占比：表算{pct_c}% ≠ 文本{pct_t}%（容忍±1pct）",
                            {"page": p},
                            "warn",
                        )
                    )
...
class R33105_FinGrantTotal_vs_Text(Rule):
    ...
    def apply(self, doc: Document) -> List[Issue]:
        ...
        if t_total and not tolerant_equal(total, t_total):
            issues.append(
                self._issue(f"财政拨款支出合计：表{total} ≠ 文本{t_total}", {"page": p}, "warn")
            )
        return issues
...
class R33107_BasicExpense_Check(Rule):
    ...
    def apply(self, doc: Document) -> List[Issue]:
        ...
        if t_total and not tolerant_equal(total, t_total):
            issues.append(
                self._issue(f"基本支出合计：表算{total} ≠ 文本{t_total}", {"page": p}, "warn")
            )
        return issues
...
class R33109_EmptyTables_Statement(Rule):
    ...
    def apply(self, doc: Document) -> List[Issue]:
        ...
        for nm in [
            ...
            if is_empty:
                if not re.search(
                    r"(空表|无相关收支|不存在|无此项|本表无数据|故本表无数据|无数据)",
                    txt_all,
                    flags=re.S | re.M,
                ):
                    issues.append(
                        self._issue(
                            f"【{nm}】为空表，但未见“空表说明/无相关收支/无数据”等说明。",
                            {"page": p},
                            "warn",
                        )
                    )
...
class R33110_BudgetVsFinal_TextConsistency(Rule):
    ...
    _NEXT_SEC = re.compile(
        r"(?m)^\s*(（四）|四、|（六）|六、|一般公共预算财政拨款基本支出决算情况说明)"
    )
    ...
    _NEGATIVE_FILTER = re.compile(
        r"同比|环比|比上年|比上年度|比上期|增长|减少|下降|较去年|与去年|上年同期|百分点|增长率|下降率|汇总|明细|简表|概览|总体情况概述"
    )
    ...
    def _apply_internal(self, doc: Document, use_ai_assist: bool = False) -> List[Issue]:
            ...
            try:
                ...
                if ai_enabled:
                    try:
                        ...
                        ai_extractor_base_url = os.getenv(
                        ...
                        if response.status_code == 200:
                            ...
                            # 转换AI结果为内部格式
                            converted_ai_pairs = self._convert_ai_pairs(
                                ai_pairs, sec_start, offsets
                            )
                            all_pairs.extend(converted_ai_pairs)
                            logger.info(
                                f"[R33110] AI结果转换完成，添加 {len(converted_ai_pairs)} 个有效结果"
                            )
                        else:
        ...
        if debug_enabled:
            regex_hits = len(
                [p for p in all_pairs if p.get("source") in ("regex", "fallback", "reverse")]
            )
    ...
    def _extract_by_rules(
        self, sec: str, sec_start: int, offsets: List[int]
    ) -> List[Dict[str, Any]]:
        """使用确定性规则抽取"""
        pairs = []
        ...
        for m in self._PAIR.finditer(sec):
            ...
            # 数字解析（支持千分位）
            try:
                float(raw_bud.replace(",", ""))
                float(raw_act.replace(",", ""))
            except Exception:
                continue

            pair_info = self._create_pair_info(
                raw_bud, raw_act, phrase, m, sec, sec_start, offsets, qizhong_pos, "regex"
            )
            if pair_info:
        ...
        for m in self._PAIR_FALLBACK.finditer(sec):
            ...
            pair_info = self._create_pair_info(
                raw_bud, raw_act, phrase, m, sec, sec_start, offsets, qizhong_pos, "fallback"
            )
            if pair_info:
                pairs.append(pair_info)
        ...
        for m in self._PAIR_REV.finditer(sec):
            ...
            pair_info = self._create_pair_info(
                raw_bud, raw_act, phrase, m, sec, sec_start, offsets, qizhong_pos, "reverse"
            )
            if pair_info:
                pairs.append(pair_info)
    ...
    def _convert_ai_pairs(
        self, ai_pairs: List[Dict[str, Any]], sec_start: int, offsets: List[int]
    ) -> List[Dict[str, Any]]:
        ...
        for ai_pair in ai_pairs:
            try:
                ...
                if not clip and ai_pair.get("stmt_text"):
                    ...
                    clip = ai_pair["stmt_text"][:50] + (
                        "..." if len(ai_pair["stmt_text"]) > 50 else ""
                    )
    ...
    def _deduplicate_pairs(self, all_pairs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for pair in all_pairs:
            ...
            for i, existing in enumerate(unique_pairs):
                # 扩大位置容差：30→60字符
                if pair["page"] == existing["page"] and abs(pair["pos"] - existing["pos"]) <= 60:
                    try:
                        # 标准化数字格式后比较
                        ...
                        if (
                            abs(float(existing_bud) - float(pair_bud)) <= bud_tol
                            and abs(float(existing_act) - float(pair_act)) <= act_tol
                        ):
                            is_duplicate = True
                            # 优先保留有reason的
                        ...
                        # 如果数字解析失败，回退到字符串比较
                        if (
                            pair["budget_text"] == existing["budget_text"]
                            and pair["final_text"] == existing["final_text"]
                        ):
                            is_duplicate = True
                            if pair.get("reason_text") and not existing.get("reason_text"):
                                unique_pairs[i] = pair
    ...
    def _generate_issues_for_pair(self, pair_info: Dict[str, Any], sec: str) -> List[Issue]:
        ...
        # 构建完整的location信息
        location_info = {"page": page, "pos": pos_in_page, "clip": clip}
...
# ---------- 构建 Document ----------
def build_document(
    path: str, page_texts: List[str], page_tables: List[List[List[List[str]]]], filesize: int
) -> Document:
    ...
    doc = Document(
        ...
        units_per_page=units,
        years_per_page=years,
    )
