...
class SmartIntelligentMerger:
    ...
    def merge_results(
        self, rule_results: List[ValidationIssue], ai_results: List[ValidationIssue]
    ...
    def _preprocess_results(
        self, results: List[ValidationIssue], source: IssueSource
    ) -> List[ValidationIssue]:
        """预处理结果：标准化和过滤"""
        processed = []
    ...
    def _calculate_similarity(self, result1: ValidationIssue, result2: ValidationIssue) -> float:
        """计算两个结果的相似度"""
        try:
            # 1. 规则ID相似度
    ...
    def _postprocess_results(self, results: List[ValidationIssue]) -> List[ValidationIssue]:
        """后处理结果"""
        # 去重
    ...
    def _is_valid_result(self, result: ValidationIssue) -> bool:
        """检查结果是否有效"""
        return result.rule_id and result.description and len(result.description.strip()) > 5
