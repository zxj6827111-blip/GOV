...
class SmartAIValidator:
        ...
        try:
            ...
            if context is None:
                context = ValidationContext(
                    document_text=document, document_hash=self._generate_hash(document)
                )
    ...
    async def _validate_rule_results(
        self, rule_results: List[ValidationIssue], context: ValidationContext
    ) -> List[ValidationIssue]:
        """验证规则结果"""
        validated_results = []
    ...
    async def _validate_single_result(
        self, result: ValidationIssue, context: ValidationContext
    ) -> ValidationAction:
        """验证单个规则结果"""
        try:
            # 构造验证提示
    ...
    async def _discover_new_issues(
        self, context: ValidationContext, existing_results: List[ValidationIssue]
    ) -> List[ValidationIssue]:
        """发现新问题"""
        try:
            # 使用AI抽取器发现新问题
            ai_hits = await self.extractor_client.ai_extract_pairs(
                context.document_text, context.document_hash
            )
    ...
    async def _enhance_descriptions(
        self, results: List[ValidationIssue], context: ValidationContext
    ) -> List[ValidationIssue]:
        """增强问题描述"""
        enhanced_results = []
...
"""
    ...
    def _convert_ai_hit_to_result(
        self, hit: Dict[str, Any], context: ValidationContext
