    ...
    async def _call_with_retry(self, section_text: str, doc_hash: str) -> List[Dict[str, Any]]:
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                last_exception = e
                logger.warning(
                    f"AI抽取超时 (尝试 {attempt + 1}/{self.config.max_retries + 1}): {e}"
                )

            except httpx.ConnectError as e:
                last_exception = e
                logger.warning(
                    f"AI抽取连接失败 (尝试 {attempt + 1}/{self.config.max_retries + 1}): {e}"
                )

            except Exception as e:
                last_exception = e
                logger.warning(
                    f"AI抽取异常 (尝试 {attempt + 1}/{self.config.max_retries + 1}): {e}"
                )
    ...
    async def _single_call(self, section_text: str, doc_hash: str) -> List[Dict[str, Any]]:
        ...
        async with httpx.AsyncClient(timeout=self.config.timeout) as client:
            response = await client.post(
                self.config.url, json=request_data, headers={"Content-Type": "application/json"}
            )

            if response.status_code != 200:
                raise Exception(
                    f"AI抽取器返回错误状态码: {response.status_code}, 响应: {response.text}"
                )
