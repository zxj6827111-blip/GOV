...
class CoreRulesEngine:
            ...
            if missing_tables:
                results.append(
                    ValidationResult(
                        ...
                        is_valid=False,
                        severity="error",
                        message=f"缺失{len(missing_tables)}张必要表格",
    ...
    def _validate_three_public_consistency(
                ...
                for keyword in self.three_public_keywords:
                    if keyword in page_text:
                        ...
                        three_public_mentions.append(
                            {"page": i + 1, "keyword": keyword, "numbers": numbers}
                        )
            ...
            if inconsistencies:
                ...
                results.append(
                    ValidationResult(
                        ...
                        is_valid=False,
                        severity="warning",
                        message=f"发现{len(inconsistencies)}处三公经费金额不一致",
                ...
                results.append(
                    ValidationResult(
                        ...
                        is_valid=True,
                        severity="info",
                        message="三公经费相关金额表述一致",
                        evidence=[],
                        page_numbers=[item["page"] for item in three_public_mentions],
                    )
                )
    ...
    def _validate_year_consistency(self, document_data: Dict[str, Any]) -> List[ValidationResult]:
        ...
        try:
            ...
            if other_years:
                ...
                problematic_years = {
                    year: pages
                    for year, pages in other_years.items()
                ...
                if problematic_years:
                    results.append(
                        ValidationResult(
                            ...
                            is_valid=False,
                            severity="warning",
                            message=f"发现与主要年份{main_year}不一致的年份",
                            evidence=[
                                {"year": year, "pages": pages}
                                for year, pages in problematic_years.items()
                            ],
                    ...
                    results.append(
                        ValidationResult(
                            ...
                            is_valid=True,
                            severity="info",
                            message=f"年份使用一致，主要年份为{main_year}",
                            evidence=[],
                            page_numbers=year_mentions[main_year][:3],
                        )
                    )
            else:
                results.append(
    ...
    def _check_amount_consistency(self, mentions: List[Dict], category: str) -> List[Dict]:
        ...
        for mention in mentions:
            if "numbers" in mention and mention["numbers"]:
                for original_text, amount, _start, _end in mention["numbers"]:
                    all_amounts.append(
                        {
