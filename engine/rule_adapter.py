...
class RuleEngineAdapter:
    ...
    def apply_all_rules(
        self, pages_text: List[str], extracted_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _build_document_from_context(self, context: ValidationContext) -> Document:
        ...
        # 估算文件大小
        filesize = context.extracted_data.get(
            "filesize", sum(len(text.encode("utf-8")) for text in page_texts)
        )
    ...
    def get_validator_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rules": [
                {"code": rule.code, "desc": rule.desc, "severity": rule.severity}
                for rule in self.rules
            ],
        }
...
class EnhancedRuleEngine:
    ...
    def validate_with_ai_assist(
        self, context: ValidationContext, use_ai_assist: bool = False
    ...
    def _enhance_with_ai(
        self, issue: ValidationIssue, context: ValidationContext
    ) -> ValidationIssue:
        """使用AI增强问题信息"""
        try:
            # 构建AI增强提示
            ...
            if ai_response.get("confidence_adjustment"):
                ...
                issue.confidence = confidence_map.get(
                    ai_response["confidence_adjustment"], issue.confidence
                )
    ...
    def _build_enhancement_prompt(self, issue: ValidationIssue, context: ValidationContext) -> str:
        """构建AI增强提示"""
        return f"""
请分析并增强以下规则检测结果：
