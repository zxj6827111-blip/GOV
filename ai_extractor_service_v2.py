...
def create_sliding_windows(
    text: str, window_size: int = WINDOW_SIZE, overlap: int = OVERLAP_SIZE
) -> List[str]:
    """创建滑窗"""
...
def adjust_spans_for_window(pairs: List[Dict[str, Any]], window_start: int) -> List[Dict[str, Any]]:
    ...
    for pair in pairs:
        ...
        adjusted_pair["budget_span"] = [
            pair["budget_span"][0] + window_start,
            pair["budget_span"][1] + window_start,
        ]
        adjusted_pair["final_span"] = [
            pair["final_span"][0] + window_start,
            pair["final_span"][1] + window_start,
        ]
        adjusted_pair["stmt_span"] = [
            pair["stmt_span"][0] + window_start,
            pair["stmt_span"][1] + window_start,
        ]

        if pair.get("reason_span"):
            adjusted_pair["reason_span"] = [
                pair["reason_span"][0] + window_start,
                pair["reason_span"][1] + window_start,
            ]
...
async def extract_v1(request: ExtractRequest):
    ...
    try:
        ...
        # 缓存检查
        f"extract_v1_{hashlib.md5(section_text.encode()).hexdigest()}"
        ...
        for i, window in enumerate(windows[: request.max_windows]):
            logger.info(
                f"处理窗口 {i + 1}/{min(len(windows), request.max_windows)}, 长度: {len(window)}"
            )
...
async def root():
    ...
    return {
        ...
        "description": "GLM和DeepSeek多模型容灾信息抽取微服务",
        "version": "2.0.0",
        "endpoints": ["/ai/extract/v1 (POST)", "/health (GET)"],
    }
