// app/app/api/jobs_adv2/[job_id]/status/route.ts
...
/**
 * 判断当前结果中是否已包含 V33-110（预算↔决算对比）问题
 */
...
/**
 * 兜底提取（Node 侧）：从 uploads/{job_id}/extracted_text.txt 提取预算↔决算对比结论
 * 返回 V33-110 风格问题列表
 */
function fallbackCompareExtract(jobId: string): any[] {
  try {
    // Next dev 进程 cwd 通常在 app 目录，uploads 在上级目录
    ...
    // 正则：匹配“年初预算为 X（万元/元），支出决算为 Y（万元/元）……（判定词）”
    // 在同一小段内寻找大于/小于/等于/持平（包含“基本持平”）
    const pairRe = new RegExp(
      [
        ...
        "\\s*(?:亿元|万元|元)?",
        "[\\s\\S]{0,240}?",
        "(决算数\\s*(?:大于|小于|等于|持平|基本持平)\\s*预算数)" // 3: 判定短语
      ].join(""),
      "g"
    );

    // 原因：在匹配尾部 200 字符内找“主要原因/增减原因/变动原因：……。”
    ...
    while ((m = pairRe.exec(txt))) {
      ...
      out.push({
        rule: "V33-110",
        severity: "error",
        message: `年初预算=${bud}，决算=${act}（文本表述：${stmt}）`,
        location: { page: 1, pos: 0, clip },
        reason_text: reason
      });

      if ((/大于|小于/.test(stmt)) && !reason) {
        out.push({
          rule: "V33-110",
          severity: "error",
          message: `文本表述为“${stmt}”，但未见主要原因说明（其中段落后）。`,
          location: { page: 1, pos: 0, clip }
        });
      }
    }
...
export async function GET(
  ...
  try {
   ...
   // 若仍无 V33-110，则本地兜底从 extracted_text 提取并注入
    try {
