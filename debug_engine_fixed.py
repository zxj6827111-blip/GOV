...
async def debug_engine_fixed():
    """修复后的引擎规则调试"""
    print("=== 修复后的引擎规则调试 ===\n")
    
    # 1. 检查可用的规则
    print("1. 检查可用规则:")
    print(f"FOCUS_COMPARE_ONLY: {os.environ.get('FOCUS_COMPARE_ONLY', '未设置')}")
    ...
    # 创建规则配置 - 使用V33格式
    rules_config = [
        {"id": "V33-001", "code": "V33-001", "title": "封面年份单位检查"},
        {"id": "V33-002", "code": "V33-002", "title": "九张表检查"},
        {"id": "V33-003", "code": "V33-003", "title": "页数阈值检查"}
    ]
    ...
    # 7. 测试规则映射
    print("\n7. 测试规则映射:")
    test_codes = ["R33001", "R33002", "V33-001", "V33-002"]
    for code in test_codes:
        normalized = runner._normalize_rule_code(code)
        ...
        for rule in ALL_RULES:
            if rule.code == normalized:
                print(f"    ✓ 找到对应规则: {rule.code} - {rule.desc}")
                found = True
                break
...
if __name__ == "__main__":
    asyncio.run(debug_engine_fixed())