...
class RuleLoaderExt:
    ...
    def _parse_rule_section(self, rules_data: Dict[str, Any], section: str) -> List[ExtendedRule]:
        ...
        for rule_id, rule_config in rules_data.items():
            ...
            desc = rule_config.get(
                "desc", rule_config.get("description", rule_config.get("title", ""))
            )
    ...
    def _parse_rule_list(
        self, rules_list: List[Dict[str, Any]], section: str
    ) -> List[ExtendedRule]:
        ...
        for idx, item in enumerate(rules_list):
            ...
            # 依据严重程度设置默认优先级
            priority_map = {"critical": 300, "error": 200, "warn": 150, "info": 100}
    ...
    async def load_rules_async(self, rules_version: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if os.path.exists(yaml_path):
                ...
                logger.warning(
                    f"Rules version '{rules_version}' not found at {yaml_path}, fallback to default"
                )
    ...
    def get_high_priority_rules(
        self, rules: List[ExtendedRule], min_priority: int = 200
    ) -> List[ExtendedRule]:
        """获取高优先级规则"""
        return self.filter_rules(rules, min_priority=min_priority)

    def sort_rules_by_priority(
        self, rules: List[ExtendedRule], descending: bool = True
    ) -> List[ExtendedRule]:
        """按优先级排序规则"""
        return sorted(rules, key=lambda x: x.priority, reverse=descending)

    def create_legacy_compatible_rules(
        self, extended_rules: List[ExtendedRule]
    ) -> List[Dict[str, Any]]:
        """创建与旧系统兼容的规则格式"""
        legacy_rules = []
