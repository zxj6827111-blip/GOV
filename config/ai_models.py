...
class AIModelManager:
    ...
    def _load_models(self):
        ...
        zhipu_flash_base_url = os.getenv(
        ...
        zhipu_glm45_base_url = os.getenv(
    ...
    def get_failover_sequence(self) -> List[ModelConfig]:
        ...
        if not sequence:
            ...
            logger.info(
                f"AI模型故障转移序列: {[f'{m.provider.value}:{m.model_name}' for m in sequence]}"
            )
    ...
    def validate_configuration(self) -> Dict[str, Any]:
        ...
        for model in available_models:
            ...
            result["providers"][provider].append(
                {"model": model.model_name, "tier": model.tier.value}
            )
...
if __name__ == "__main__":
    ...
    for i, model in enumerate(models, 1):
        print(
            f"{i}. {model.provider.value}:{model.model_name} ({model.tier.value}) - {model.base_url}"
        )
