...
class IssueItem(BaseModel):
    ...
    def create_id(cls, source: str, rule_id: str, location: Dict[str, Any]) -> str:
        """生成唯一ID"""
        loc_str = (
            f"{location.get('page', 0)}_{location.get('section', '')}_{location.get('table', '')}"
        )
...
class ConflictItem(BaseModel):
    ...
    reason: Literal["value-mismatch", "missing", "page-mismatch"] = Field(
        ..., description="冲突原因"
    )
...
class DualModeResponse(BaseModel):
    ...
    # 兼容性字段（camelCase适配）
    aiFindings: Optional[List[IssueItem]] = Field(
        default=None, description="AI检查结果（camelCase）"
    )
    ruleFindings: Optional[List[IssueItem]] = Field(
        default=None, description="规则检查结果（camelCase）"
    )
...
class AnalysisConfig(BaseModel):
    ...
    enable_ai_locator: bool = Field(
        default=True, description="是否启用AI定位增强（将AI帮助用于定位证据）"
    )
    ai_fallback_on_error: bool = Field(
        default=True, description="AI分析失败时是否静默回退到仅规则模式"
    )
...
def create_issue_item(
    title: str, rule_id: str, source: str, page_number: int, **kwargs
) -> IssueItem:
    """创建问题项的便捷函数"""
    return IssueItem(
